...
class GoogleService {
  ...
  initializeGoogle() {
    try {
      ...
      console.log('✅ Google 服務已初始化');
    } catch (e) {
      console.log('⚠️ Google 初始化失敗，需要授權');
    }
  }
  ...
  async setTokens(code) {
    try {
      ...
      db.set('google_tokens', tokens).write();
      this.tokens = tokens;
      this.initializeGoogle();
      return '✅ Google 授權成功！';
    } catch (e) {
      return `❌ 授權失敗：${e.message}`;
    }
  }

  async getUpcomingEvents(days = 1) {
    if (!this.initialized) return '❌ Google 日曆未授權';
    ...
    try {
      ...
      if (events.length === 0) return '📭 接下來沒有日程';

      let msg = `📅 **接下來 ${days} 天的日程**\n\n`;
      events.forEach((event, i) => {
        const start = new Date(event.start.dateTime || event.start.date).toLocaleString('zh-TW');
        msg += `${i+1}. **${event.summary}**\n   ⏰ ${start}\n`;
        if (event.description) msg += `   📝 ${event.description.substring(0, 50)}\n`;
        msg += '\n';
      });

      return msg;
    } catch (e) {
      return `❌ 日曆查詢失敗：${e.message}`;
    }
  }

  async getUnreadEmails(limit = 5) {
    ...
    try {
      ...
      if (messages.length === 0) return '📭 沒有未讀郵件';

      let msg = `📬 **未讀郵件（共 ${messages.length} 封）**\n\n`;
