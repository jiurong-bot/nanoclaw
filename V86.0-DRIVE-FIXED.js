// V86.0-DRIVE-FIXED - 完全干淨版本（無重複、無錯誤）
// 部署日期：2026-02-25
...
const VERSION = "V86.0-DRIVE-FIXED";
...
db.defaults({
  ...
  alerts: [],
  token_usage: [],
  personality: { big5: { openness: 50, conscientiousness: 50, extraversion: 50, agreeableness: 50, neuroticism: 50 }, speaking_style: 'balanced', learned_responses: {} },
  google_tokens: null,
  classified_logs: [],
  email_context: null,
...
class EnhancedAutoClassifier {
  ...
  async classifyAndSave(topic, content, category) {
    ...
    try {
      ...
      classified.push({ timestamp: Date.now(), topic, category, content: content.substring(0, 100) });
...
class EnhancedNLPIntentDetector {
  constructor() {
    this.intents = {
      ...
      search: ['搜尋', '搜索'], help: ['幫助', 'help'], status: ['狀態', 'status'], monitor: ['監控', 'monitor']
    };
  }
...
class GoogleDriveService {
  ...
  async uploadFile(filePath, fileName = null) {
    ...
    try {
      ...
      const res = await this.drive.files.create({
        requestBody: { name: name, mimeType: 'application/octet-stream' },
        media: { mimeType: 'application/octet-stream', body: fileContent }
      });
  ...
  async downloadFile(fileId, savePath) {
    if (!this.initialized) return '❌ Drive 未授權';
    try {
      const res = await this.drive.files.get({ fileId: fileId }, { responseType: 'stream' });
      ...
      for (const file of files) {
        if (file.endsWith('.md')) {
          ...
          await this.drive.files.create({
            requestBody: { name: `memory-backup-${file}`, parents: ['root'] },
  ...
  async deleteEmail(emailId) {
    ...
    try {
      await this.gmail.users.messages.delete({ userId: 'me', id: emailId });
      return '✅ 已刪除';
    } catch (e) {
      return `❌ 失敗`;
    }
...
class TokenMonitor {
  ...
  recordUsage(model, inputTokens, outputTokens) {
    ...
    const record = { timestamp: Date.now(), totalTokens, cost: parseFloat(cost.toFixed(6)) };
